...
export async function checkTourDone(userId?: string): Promise<boolean> {
  if (localStorage.getItem('shh_tour_done')) return true
  ...
  try {
    const { data } = await supabase
      ...
      .single()
    if (data?.tour_done) {
      localStorage.setItem('shh_tour_done', '1') // cache locally for next time
      return true
    }
  } catch {}
  return false
...
// ── Mark tour done — saves to both localStorage and Supabase ──
export async function markTourDone(userId?: string): Promise<void> {
  localStorage.setItem('shh_tour_done', '1')
...
function LoadingScreen({ message, name }: { message: string; name?: string }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'var(--bg)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', zIndex:9999, overflow:'hidden' }}>
      <div style={{ position:'absolute', inset:0, background:'radial-gradient(ellipse 80% 60% at 50% 40%, rgba(80,100,220,0.08) 0%, transparent 70%)', pointerEvents:'none' }}/>
      ...
      {phase >= 2 && particles.map((p, i) => (
        <div key={i} style={{ position:'absolute', left:p.x, top:p.y, width:p.size, height:p.size, borderRadius:'50%', background:i%2===0?'var(--accent)':'#b07ef7', opacity:0.5, animation:`particleFloat ${p.dur}s ease-in-out infinite`, animationDelay:`${p.delay}s`, filter:'blur(0.5px)', boxShadow:`0 0 ${p.size*2}px var(--accent-glow)` }}/>
      ))}

      <div style={{ position:'relative', marginBottom:44, zIndex:1, animation:'logoEntrance 0.7s var(--ease-out) both' }}>
        <div style={{ position:'absolute', inset:-20, borderRadius:'50%', border:'1px solid var(--border-active)', animation:'ringExpand 2.4s ease-out infinite', animationDelay:'0.2s' }}/>
        <div style={{ position:'absolute', inset:-20, borderRadius:'50%', border:'0.5px solid rgba(99,140,245,0.25)', animation:'ringExpand 2.4s ease-out infinite', animationDelay:'1.4s' }}/>
        <div style={{ position:'absolute', inset:-12, borderRadius:'50%', border:'1px solid rgba(99,140,245,0.2)', boxShadow:'0 0 24px rgba(99,140,245,0.15), inset 0 0 24px rgba(99,140,245,0.05)' }}/>
        <div style={{ width:72, height:72, borderRadius:22, background:'linear-gradient(135deg, var(--accent) 0%, #7b6cf6 50%, #b07ef7 100%)', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 0 0 1px rgba(255,255,255,0.12) inset, 0 16px 48px var(--accent-glow), 0 4px 16px rgba(0,0,0,0.3)', animation:'logoBreath 3s ease-in-out infinite', animationDelay:'0.8s', position:'relative', zIndex:1 }}>
      ...
      {phase >= 1 && (
        <div style={{ zIndex:1, textAlign:'center', animation:'textReveal 0.55s var(--ease-out) both' }}>
          {firstName ? (
            <>
              <div style={{ fontFamily:'var(--font-display)', fontSize:14, letterSpacing:'2px', textTransform:'uppercase', color:'var(--text-3)', marginBottom:6, animation:'textReveal 0.55s var(--ease-out) 0.05s both' }}>{greeting}</div>
              <div style={{ fontFamily:'var(--font-display)', fontSize:32, letterSpacing:'-0.8px', color:'var(--text-1)', marginBottom:10, lineHeight:1.1, animation:'textReveal 0.6s var(--ease-out) 0.1s both' }}>
                {firstName} <span style={{ fontStyle:'italic', color:'var(--accent)' }}>✦</span>
              </div>
            </>
          ) : (
            <div style={{ fontFamily:'var(--font-display)', fontSize:28, letterSpacing:'-0.5px', color:'var(--text-1)', marginBottom:10 }}>Shhhhh</div>
          )}
        </div>
      )}

      {phase >= 1 && (
        <div style={{ fontSize:13, color:'var(--text-3)', fontWeight:300, letterSpacing:'0.2px', zIndex:1, animation:'textReveal 0.55s var(--ease-out) 0.1s both', minHeight:20 }}>{message}</div>
      )}

      {phase >= 1 && (
        <div style={{ display:'flex', gap:7, marginTop:32, zIndex:1 }}>
          {[0,1,2].map(i => (
            <div key={i} style={{ width:5, height:5, borderRadius:'50%', background:'var(--accent)', animation:'pulse 1.2s ease-in-out infinite', animationDelay:`${i*0.2}s`, boxShadow:'0 0 8px var(--accent-glow)' }}/>
          ))}
        </div>
      )}

      <div style={{ position:'fixed', bottom:0, left:0, right:0, height:2, background:'var(--text-4)', overflow:'hidden' }}>
        <div style={{ height:'100%', width:'45%', background:'linear-gradient(90deg, transparent, var(--accent), #b07ef7, transparent)', animation:'loadSweep 1.6s ease-in-out infinite', boxShadow:'0 0 12px var(--accent-glow)' }}/>
      </div>
    </div>
  )
}
...
function Landing({ onStart }: { onStart: () => void }) {
  const [dark, setDark] = useState(() => (localStorage.getItem('shh_theme') ?? 'dark') === 'dark')
  const toggleTheme = () => {
      ...
      <div style={{ maxWidth:800, margin:'0 auto', padding:'0 clamp(20px,5vw,48px) 80px', position:'relative', zIndex:1 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'28px 0 0' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <button onClick={toggleTheme} title={dark?'Light mode':'Dark mode'} style={{ width:36, height:36, borderRadius:'50%', border:'0.5px solid var(--border)', background:'var(--bg-card)', color:'var(--text-2)', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', transition:'all .2s' }}>
              {dark
            ...
            <button onClick={onStart} style={{ padding:'8px 20px', borderRadius:999, border:'0.5px solid var(--border)', background:'var(--bg-card)', color:'var(--text-2)', fontSize:13, cursor:'pointer', fontFamily:'var(--font-body)', transition:'all .2s' }}>Sign in</button>
          </div>
        </div>
        <div style={{ textAlign:'center', padding:'72px 0 56px' }}>
          ...
          <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
            <button onClick={onStart} style={{ padding:'14px 32px', borderRadius:999, background:'linear-gradient(135deg,var(--accent),#7b6cf6)', border:'none', color:'white', fontSize:15, fontWeight:500, cursor:'pointer', fontFamily:'var(--font-body)', boxShadow:'0 6px 24px var(--accent-glow)', transition:'all .2s' }}>Get started free →</button>
            <button onClick={onStart} style={{ padding:'14px 24px', borderRadius:999, border:'0.5px solid var(--border)', background:'var(--bg-card)', color:'var(--text-2)', fontSize:15, cursor:'pointer', fontFamily:'var(--font-body)', transition:'all .2s' }}>Sign in</button>
          </div>
        ...
        <div style={{ textAlign:'center', background:'var(--bg-card)', border:'0.5px solid var(--border)', borderRadius:24, padding:'40px 32px' }}>
          <div style={{ fontFamily:'var(--font-display)', fontSize:'clamp(20px,3vw,28px)', letterSpacing:'-0.5px', color:'var(--text-1)', marginBottom:10 }}>Ready to study smarter?</div>
          <p style={{ fontSize:13, color:'var(--text-3)', fontWeight:300, marginBottom:24 }}>Join the beta. Free. Your feedback shapes what gets built next.</p>
          <button onClick={onStart} style={{ padding:'12px 36px', borderRadius:999, background:'linear-gradient(135deg,var(--accent),#7b6cf6)', border:'none', color:'white', fontSize:14, fontWeight:500, cursor:'pointer', fontFamily:'var(--font-body)', boxShadow:'0 4px 18px var(--accent-glow)', transition:'all .2s' }}>Start now — it's free</button>
        </div>
        <p style={{ textAlign:'center', fontSize:11, color:'var(--text-3)', marginTop:28, fontWeight:300 }}>
          Built with love by Svulele — for someone who inspired it.<br/>Your data stays on your device.
        </p>
      </div>
    </div>
  )
  ...
  if (mode === 'confirm') return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', padding:24, background:'var(--bg)' }}>
      <div style={{ width:'100%', maxWidth:400, background:'var(--bg-card)', border:'0.5px solid var(--border)', borderRadius:28, padding:'40px 32px', textAlign:'center', animation:'pageUp .4s var(--ease-out) both' }}>
        ...
        <button onClick={()=>setMode('login')} style={{ padding:'11px 28px', borderRadius:999, background:'linear-gradient(135deg,var(--accent),#7b6cf6)', border:'none', color:'white', fontSize:14, fontWeight:500, cursor:'pointer', fontFamily:'var(--font-body)', boxShadow:'0 4px 18px var(--accent-glow)' }}>Go to sign in →</button>
    ...
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', padding:24, background:'var(--bg)', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ width:'100%', maxWidth:400, background:'var(--bg-card)', border:'0.5px solid var(--border)', borderRadius:28, padding:'36px 32px 32px', position:'relative', zIndex:1, animation:'pageUp .4s var(--ease-out) both' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:28 }}>
          ...
          <div style={{ fontSize:13, color:'var(--text-3)', fontWeight:300 }}>{mode==='login'?'Welcome back. Ready to learn?':'Create your free account.'}</div>
        </div>
...
// ── USER_KEYS — shh_tour_done intentionally excluded ─────────
// Tour completion persists across sign-outs via Supabase profile.
const USER_KEYS = [
  'shh_profile', 'shh_books', 'shh_notes', 'shh_chat_sessions',
  'shh_flashcards', 'shh_memory', 'shh_reading_ctx', 'shh_session',
  'shh_streak', 'shh_study_time', 'shh_quote_state', 'shh_skip_auth',
  'shh_daily_plan',
]
...
export default function AuthGate({ children }: Props) {
  ...
  // FIX: resolved tour status is passed to children via a separate prop
  const [tourDone, setTourDone] = useState<boolean>(true) // default true = don't flash tour
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      ...
      if (event === 'SIGNED_OUT' || !u) {
        ...
        setUser(null)
        setTourDone(true) // reset so next user gets checked
        setScreen('landing')
        return
      }
      if (currentUid.current && currentUid.current !== u.id) clearUserData()
      if (currentUid.current !== u.id) handleReady(u)
    })
  ...
  const handleReady = async (u: User) => {
    ...
    setScreen('loading')
    const savedName = (() => { try { return JSON.parse(localStorage.getItem('shh_profile') ?? '{}').name ?? '' } catch { return '' } })()
    setName(savedName)
    ...
    try {
      ...
      const syncedName = (() => { try { return JSON.parse(localStorage.getItem('shh_profile') ?? '{}').name ?? '' } catch { return '' } })()
      if (syncedName) setName(syncedName)
    } catch {}

    // FIX: check Supabase + localStorage BEFORE showing app
    // This prevents tour showing on sign-in #2, #3, new devices
    const done = await checkTourDone(u.id)
    setTourDone(done)
  ...
  // Pass tourDone down through the render prop so App.tsx can use it
  if (screen === 'app' && user) return <>{children(user, doSignOut, tourDone)}</>
