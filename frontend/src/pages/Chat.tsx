...
function addStudyTime(secs:number) {
  ...
  try {
    ...
    const t: Record<string,number> = {}; keys.forEach(k=>{t[k]=d[k]})
...
// 1. Check live session (set when Library reader is open)
// 2. Fall back to most recently read book from shh_books
function getReadingContext() {
  try {
    ...
    // Fall back to most recently opened book
    const books = JSON.parse(localStorage.getItem('shh_books') ?? '[]')
    if (!books.length) return null
    // Most recently read = highest currentPage relative to when it was last touched
    const recent = [...books]
      .filter((b: any) => b.currentPage > 1)
      .sort((a: any, b: any) => b.addedAt - a.addedAt)[0]
...
const VIBE_CONFIG: Record<string, { system:string; greeting:(n:string)=>string; starters:string[] }> = {
  gentle: {
    system: `Your tone is warm, gentle and deeply patient. Celebrate every small win. Never criticise. Use encouraging language. Ask soft check-in questions. Think: the kindest teacher they ever had.`,
    greeting: n=>`Hey ${n} 🌱 No pressure — what's on your mind?`,
    starters: ["I'm struggling, can you help?","Explain this gently","I feel stuck — where do I start?","Can you encourage me?"],
  },
  balanced: {
    system: `Warm but focused. Balance encouragement with accountability. Ask follow-up questions. Like a trusted study partner who is supportive but honest.`,
    greeting: n=>`Hey ${n}! What are we working on today?`,
    starters: ["Explain what I just read","Quiz me on this","Help me make a study plan","What should I focus on?"],
  },
  strict: {
    system: `Direct and results-focused. Get to the point. Push back on vague answers. Short, sharp sentences. No fluff. Think: demanding but fair coach.`,
    greeting: n=>`${n}. What are we working on? Let's not waste time.`,
    starters: ["Test me — no easy questions","Give me a hard problem","Tell me what I got wrong","Push me harder"],
  },
  chill: {
    system: `Relaxed, casual, pressure-free. Like a smart friend on the couch. Occasional emoji. Don't make them feel bad.`,
    greeting: n=>`Hey ${n} 👋 What's up?`,
    starters: ["Explain this like I'm half asleep","Short version please","What's actually important here?","Can we just talk through this?"],
  },
}

function buildSystem(material: any): string {
  const p      = getProfile()
  ...
  // Get live reading context from localStorage
  const readCtx = material?.book
  ...
  let sys = `You are Shhhhh, an AI study buddy. ${name} ${goals} Vibe: ${vibe}.

PERSONALITY: ${cfg.system}

IMPORTANT RULES:
- You ALWAYS know what the user is currently reading (see context below)
- When asked about connections, summaries, or explanations — reference the specific book and pages they're on
- Never ask "what are you studying?" or "what book?" — you already know from context
- If they ask "how does it connect?" or "what came before?" — reference the actual book content based on your knowledge of it
- Stay in character for "${vibe}" vibe at all times
- Be a companion, not just a tool — ask follow-ups, show genuine interest
- Keep responses concise. Use markdown for lists/code`

  if (readCtx) {
    const pctDone = readCtx.totalPages > 0 ? Math.round((readCtx.currentPage / readCtx.totalPages) * 100) : 0
    sys += `

CURRENT READING SESSION:
- Book: "${readCtx.title}" by ${readCtx.author}
- Currently on: page ${readCtx.currentPage} of ${readCtx.totalPages} (${pctDone}% complete)
- This means they are reading content from approximately page ${Math.max(1, readCtx.currentPage - 10)} to ${readCtx.currentPage} right now

Use your knowledge of "${readCtx.title}" to:
- Know what topics pages ${readCtx.currentPage - 20}–${readCtx.currentPage} likely cover
- Explain what came before the current section
- Connect current content to earlier chapters
- Answer questions about the specific content they just read`
  }

  if (material?.recap) {
    const r = material.recap
    sys += `

JUST COMPLETED: reading session pages ${r.fromPage}–${r.toPage}.
Session summary: ${r.summary?.join(' ')}`
  }
  ...
  return (
    <div style={{display:'flex',justifyContent:isUser?'flex-end':'flex-start',marginBottom:14,gap:8,alignItems:'flex-start'}}>
      ...
      <div style={{maxWidth:'72%',padding:'12px 16px',fontSize:14,color:'var(--text-1)',fontWeight:300,borderRadius:isUser?'18px 4px 18px 18px':'4px 18px 18px 18px',background:isUser?'var(--accent-soft)':'var(--bg-card)',border:`0.5px solid ${isUser?'var(--border-active)':'var(--border)'}`}}>
        {hasBullets?<ul style={{paddingLeft:16,margin:0}}>{lines.map(render)}</ul>:lines.map(render)}
      </div>
      {!isUser&&!streaming&&(
        <button onClick={onSpeak} title="Read aloud" style={{background:'none',border:'none',cursor:'pointer',color:'var(--text-3)',padding:4,display:'flex',alignItems:'center',marginTop:6,flexShrink:0}}>
...
export default function Chat({ material }:{ material:any }) {
  ...
  const [curVibe,   setCurVibe]   = useState(()=>getProfile().vibe??'balanced')
  ...
  useEffect(()=>{
    ...
    return ()=>{ const secs=Math.round((Date.now()-sessionStart.current)/1000); if(secs>10) addStudyTime(secs) }
  },[])
  ...
  useEffect(()=>{
    const iv = setInterval(()=>{ const v=getProfile().vibe??'balanced'; setCurVibe(v) },2000)
    return ()=>clearInterval(iv)
  },[])

  const badge  = VIBE_BADGE[curVibe]??VIBE_BADGE.balanced
  const cfg    = VIBE_CONFIG[curVibe]??VIBE_CONFIG.balanced
  const p      = getProfile()

  // Get current reading context for display
  const readCtx = material?.book
    ? { title: material.book.title, author: material.book.author, currentPage: material.book.currentPage }
    : getReadingContext()

  const newSess = ():Session => ({
    id:Date.now().toString(),
    title: readCtx ? `Chat — ${readCtx.title}` : 'New chat',
    messages:[], createdAt:Date.now(),
    bookCtx: readCtx?.title, vibe: curVibe,
  })
  ...
  const sendMsg = async (text?:string) => {
    ...
    // Build system fresh each send — vibe, name, goals, current page all live
    const sys = buildSystem(material)
    ...
    let sid = activeId
    if(!sid) { const s=newSess(); s.messages=updated; setSessions(prev=>[s,...prev]); setActiveId(s.id); sid=s.id }
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',flex:1,minHeight:0,overflow:'hidden'}}>
      ...
      {/* Topbar */}
      <div style={{display:'flex',alignItems:'center',gap:10,padding:'11px clamp(14px,3vw,32px)',borderBottom:'0.5px solid var(--border)',background:'var(--bg-card)',backdropFilter:'blur(12px)',flexShrink:0}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontFamily:'var(--font-display)',fontSize:18,letterSpacing:'-0.4px',color:'var(--text-1)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>
            Ask the AI
          </div>
          {readCtx && (
            <div style={{display:'flex',alignItems:'center',gap:5,marginTop:2}}>
              <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="10"/></svg>
              <span style={{fontSize:10,color:'var(--accent)',fontWeight:500}}>
                {readCtx.title} · p.{readCtx.currentPage}/{readCtx.totalPages}
              </span>
            </div>
          )}
        </div>
        <div style={{display:'flex',alignItems:'center',gap:5,padding:'4px 10px',borderRadius:999,background:'var(--bg-card)',border:'0.5px solid var(--border)',fontSize:11,color:badge.color,flexShrink:0}}>
          <span>{badge.emoji}</span><span style={{fontWeight:500}}>{badge.label}</span>
        </div>
        <button onClick={()=>setShowHist(h=>!h)} style={{display:'flex',alignItems:'center',gap:5,padding:'5px 11px',borderRadius:999,border:`0.5px solid ${showHist?'var(--border-active)':'var(--border)'}`,background:showHist?'var(--accent-soft)':'var(--bg-card)',color:showHist?'var(--accent)':'var(--text-2)',fontSize:12,cursor:'pointer',fontFamily:'var(--font-body)',flexShrink:0,transition:'all .18s'}}>
        ...
        <button onClick={()=>{const s=newSess();setSessions(prev=>[s,...prev]);setActiveId(s.id);setMessages([]);setError(null)}} style={{display:'flex',alignItems:'center',gap:5,padding:'5px 11px',borderRadius:999,border:'0.5px solid var(--border)',background:'var(--bg-card)',color:'var(--text-2)',fontSize:12,cursor:'pointer',fontFamily:'var(--font-body)',flexShrink:0,transition:'all .18s'}}>
      ...
      <div style={{display:'flex',flex:1,minHeight:0,overflow:'hidden'}}>
        {showHist&&(
          <div style={{width:230,borderRight:'0.5px solid var(--border)',background:'var(--bg-card)',display:'flex',flexDirection:'column',flexShrink:0,overflow:'hidden'}}>
            <div style={{padding:'11px 14px 6px',fontSize:10,letterSpacing:'2.5px',textTransform:'uppercase',color:'var(--text-3)'}}>Recent chats</div>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',minHeight:0,minWidth:0}}>
            ...
            {isEmpty&&(
              <div style={{maxWidth:500,margin:'0 auto',paddingTop:28}}>
                <div style={{fontFamily:'var(--font-display)',fontSize:21,letterSpacing:'-0.4px',color:'var(--text-1)',marginBottom:6}}>
                  {cfg.greeting(p.name?.split(' ')[0]||'there')}
                </div>
                {readCtx&&<div style={{fontSize:13,color:'var(--text-3)',marginBottom:22,fontWeight:300,lineHeight:1.6}}>
                  I know you're on <span style={{color:'var(--accent)'}}>p.{readCtx.currentPage}</span> of <span style={{color:'var(--accent)'}}>{readCtx.title}</span> — ask me anything about it.
                </div>}
          ...
          {/* Input */}
          <div style={{borderTop:'0.5px solid var(--border)',background:'var(--bg-card)',backdropFilter:'blur(16px)',flexShrink:0,padding:`10px clamp(14px,3vw,36px) ${CSS.supports('padding-bottom','env(safe-area-inset-bottom)')?'calc(80px + env(safe-area-inset-bottom))':'80px'}`}}>
            <div style={{display:'flex',gap:8,alignItems:'flex-end',maxWidth:720,margin:'0 auto'}}>
              ...
              <textarea ref={inputRef} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendMsg()}}}
                placeholder={listening?'Listening…':`Ask ${badge.emoji} anything…`}
                rows={1} disabled={streaming||listening}
                style={{flex:1,minHeight:40,maxHeight:110,resize:'none',fontSize:14,fontWeight:300,borderRadius:12,padding:'10px 13px',lineHeight:1.5}}/>
